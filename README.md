...
13. **進捗ダッシュボード** … 対象社員の予約済み/未予約・問診提出済み/未提出の人数と割合、
    巡回健診日ごとの予約充足率をひと目で把握。
14. **未対応者の抽出・出力** … 未予約者・問診未提出者を一覧表示し、CSV/XLSXで出力（催促連絡用）。
15. **当日の受付名簿** … 巡回健診日ごとに氏名・社員番号・時間・コース・オプションを含む名簿を CSV/XLSX 出力。
16. **問診結果の一括出力** … 健診機関向けに受診者識別情報＋健診日時＋全問診回答をまとめて出力（健診日で絞り込み可）。
